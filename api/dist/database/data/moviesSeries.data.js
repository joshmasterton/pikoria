...
export const storeMoviesSeries = async (minReleaseYear) => {
    const client = await pool.connect();
    const { TMDB_URL, TMDB_API_KEY } = process.env;
    try {
        const res = await client.query(`SELECT movies_series_stored from setup_status WHERE id = 1`);
        ...
        const fetchData = async (type) => {
            ...
            while (page <= total_pages) {
                const response = await axios.get(`${TMDB_URL}/discover/${type === "movie" ? "movie" : "tv"}`, {
                    params: {
                        ...params,
                        page,
                    },
                ...
                console.log(`   Total results: ${total_results}, Total pages: ${total_pages}`);
                const query = `
