...
import { checkLikedStatus, insertFavouriteMovieSeries, } from "../database/models/moviesSeries.model.js";
export const processMoviesSeriesRecommendation = async (recommendationData, user_id) => {
    try {
        // TMDB API key
        const { TMDB_API_KEY, TMDB_URL } = process.env;
        // Movies or series
        const endpoint = recommendationData.content === "movies"
            ? "/discover/movie"
            : "/discover/tv";
        // Get TMDB movies_series based off user forms
        const params = recommendationData.content === "movies"
            ? {
        ...
        const TMDBMoviesSeriesWithLike = await Promise.all(TMDBResponse.data.results.map(async (movieSeries) => {
        ...
        return {
            ...
            total_pages: TMDBResponse.data.total_pages ?? 0,
            total_results: TMDBResponse.data.total_results ?? 0,
        };
    }
    catch (error) {
...
export const processLikeMovieSeries = async (user_id, favouriteMovieSeries) => {
    try {
